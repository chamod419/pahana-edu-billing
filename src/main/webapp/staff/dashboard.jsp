...
<head>
    ...
    <style>
        ...
        :root{
            ...
            --shadow-xs:   0 0 0 1px rgba(30,41,59,0.06);
        }
        ...
        *{margin:0;padding:0;box-sizing:border-box}
        ...
        @keyframes floaty{ 0%{ transform: translate3d(0,0,0) rotate(0deg);} 50%{ transform: translate3d(20px,-10px,0) rotate(0.2deg);} 100%{ transform: translate3d(0,0,0) rotate(0deg);} }
        ...
        .topbar-content{ display:flex; justify-content:space-between; align-items:center; position:relative; z-index:1; }
        ...
        .staff-badge{
            ...
            padding: 4px 12px; border-radius: 20px; font-size: 12px; font-weight: 700; text-transform: uppercase; letter-spacing: .5px;
        }

        .logout-btn{
            background: rgba(255,255,255,0.18); color: var(--white); text-decoration:none;
            padding:8px 16px; border-radius:8px; font-weight:600; transition: all .3s ease; backdrop-filter: blur(10px);
            border:1px solid rgba(255,255,255,0.22);
        }
        .logout-btn:hover{ background: rgba(255,255,255,0.28); transform: translateY(-1px); box-shadow: var(--shadow-sm); }

        .topbar-center{ position:absolute; left:50%; top:50%; transform: translate(-50%,-50%); z-index:2; pointer-events:none; }
        .logo{ width:56px; height:56px; margin:0 auto; background: var(--white); border-radius:50%; display:flex; align-items:center; justify-content:center; box-shadow: var(--shadow-sm); padding:8px; }
        .logo img{ max-width:40px; max-height:40px; object-fit:contain; display:block; }
        .logo-fallback{ display:none; color: var(--midnight); font-size:20px; font-weight:700; }

        .stats-grid{ display:grid; grid-template-columns: repeat(auto-fit, minmax(280px,1fr)); gap:24px; margin-bottom:40px; }
        .stat-card{
            background: var(--white); border-radius:16px; padding:24px; position:relative; overflow:hidden;
            box-shadow: var(--shadow-sm), var(--shadow-xs); transition: transform .25s ease, box-shadow .25s ease;
        }
        .stat-card::before{
            content:''; position:absolute; left:0; right:0; top:0; height:4px;
            background: linear-gradient(90deg, var(--midnight), var(--teal), var(--deep-navy), var(--forest));
            background-size: 200% 100%; animation: band 4s ease infinite;
        }
        ...
        .stat-icon{ position:absolute; top:20px; right:20px; width:48px; height:48px; border-radius:12px; display:flex; align-items:center; justify-content:center; font-size:24px; opacity:.85; }
        ...
        .nav-section{ background: var(--white); border-radius:16px; padding:32px; box-shadow: var(--shadow-sm), var(--shadow-xs); margin-bottom:32px; }
        .nav-title{ font-size:20px; font-weight:800; color: var(--midnight); margin-bottom:24px; padding-bottom:12px; border-bottom:2px solid var(--light-gray); }
        .nav-grid{ display:grid; grid-template-columns: repeat(auto-fit, minmax(200px,1fr)); gap:16px; }

        .nav-link{
            display:flex; align-items:center; gap:12px; padding:16px 20px; border-radius:12px; background: var(--light-gray);
            border:2px solid transparent; text-decoration:none; color: var(--dark); font-weight:700; transition: all .25s ease; position:relative; overflow:hidden;
        }
        .nav-link::before{ content:''; position:absolute; inset:0; left:-100%; background: linear-gradient(90deg, transparent, rgba(15,118,110,0.12), transparent); transition:left .45s ease; }
        .nav-link:hover{ color: var(--teal); background: #f0fdfa; border-color: var(--teal); box-shadow: 0 8px 18px rgba(15,118,110,0.15); transform: translateY(-2px); }
        .nav-link:hover::before{ left:100%; }
        .nav-link.primary{ background: linear-gradient(135deg, var(--midnight) 0%, var(--teal) 100%); color: var(--white); }
        .nav-link.primary:hover{ background: linear-gradient(135deg, var(--deep-navy) 0%, var(--emerald) 100%); border-color: transparent; box-shadow: 0 8px 22px rgba(30,58,138,0.28); }
        .nav-icon{ font-size:20px; }

        .help-section .nav-title{ color: var(--forest); }
        .help-section .nav-title::before{ content: '❓ '; color: var(--gold); }

        .faq-container{ display: grid; gap: 12px; }
        .faq-item{ border: 2px solid var(--light-gray); border-radius: 12px; overflow: hidden; transition: all .3s ease; }
        .faq-item:hover{ border-color: var(--teal); box-shadow: 0 4px 15px rgba(15,118,110,0.1); }
        .faq-question{
            background: var(--light-gray); padding: 16px 20px; cursor: pointer; font-weight: 700; color: var(--midnight);
            display: flex; justify-content: space-between; align-items: center; transition: background-color .3s ease;
        }
        .faq-question:hover{ background: #f0fdfa; }
        .faq-answer{ padding: 20px; background: var(--white); display: none; border-top: 2px solid var(--light-gray); }
        .faq-answer.show{ display: block; }
        .faq-answer ul{ margin: 10px 0; padding-left: 20px; }
        .faq-answer li{ margin: 5px 0; color: var(--slate); }

        .kbd{ background: var(--light-gray); border: 1px solid var(--slate); border-radius: 4px; padding: 2px 6px; font-family: 'Courier New', monospace; font-size: 12px; color: var(--midnight); font-weight: 600; }
        .faq-icon{ transition: transform .3s ease; color: var(--teal); }
        ...
        <div class="stats-grid">
            ...
            <div class="stat-card">
                ...
                <div class="stat-number">
                    Rs. <fmt:formatNumber value="${empty staffTodayRevenue ? 0 : staffTodayRevenue}" minFractionDigits="2" maxFractionDigits="2"/>
                </div>
            </div>
            <div class="stat-card">
